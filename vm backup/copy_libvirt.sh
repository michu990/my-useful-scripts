...
if [ ! -d "/var/lib/libvirt" ]; then
    echo -e "$(red "Folder źródłowy /var/lib/libvirt nie istnieje.")"
    exit 1
fi

# Confirm
clear_history
echo -e "$(green "Wybrana ścieżka docelowa:") $destination_path"
...
# CP
echo -e "$(green "Kopiowanie zawartości /var/lib/libvirt do $destination_path")"
